...
import { clearInfo, selectInfo } from "@/redux/features/user/userSlice";
...
export default function UserDropdown() {
  ...
  useEffect(() => {
    ...
    fetchImage();
  }, [session?.user.access_token, userInfo]);

  const logout = () => {
    const promise = authLogout({})
      .unwrap()
      .then(async (res) => {
        if (!res.error) {
          dispatch(clearInfo());
    ...
    toast.promise(promise, {
      loading: "Loading",
      error: () => {
        return `Keluar gagal, coba beberapa saat lagi!`;
      },
    });
  };
    ...
    <div className="relative">
      ...
      >
        <Image
          ...
          alt="User"
          className="mr-3 object-cover rounded-full h-11 w-11"
        />
